 ...
 * Draws a full A4 (300dpi) calendar image of April–June 2026.
 */
 ...
 * @param {number} zone     - zone number (for subtitle)
 * @param {string} zoneLabel - e.g. "Zone 4"
 */
export function drawCalendarToCanvas(events, zone = 4, zoneLabel = 'Zone 4') {
  const A4W = 2480, A4H = 3508, MG = 100;
  ...
  ctx.textAlign = 'left';
  ctx.fillText('April \u2013 June 2026', MG, cy + 56);
  cy += 78;

  // Subtitle
  ctx.fillStyle = '#5F5E5A';
  ctx.font = `400 32px ${FONT}`;
  ctx.fillText(`IGCSE Exam Planner  \u00b7  Cambridge M/J 2026  \u00b7  ${zoneLabel}`, MG, cy + 32);
  cy += 52;
  ...
  MONTHS.forEach((md, mi) => {
    ...
    for (let d = 1; d <= total; d++) {
      ...
      const cx2 = gx + col * cellW, cy2 = cy + row * CELL_H;
      const k = dateKey(md.y, md.m, d);
      const evs = events.filter(e => e.date === k);
      const vis = evs.slice(0, 4), extra = evs.length - 4;
